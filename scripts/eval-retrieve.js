...
// against this repo's own codebase-intel index and measures precision, MRR, and
...
function printHeader(dataset, root, healthInfo) {
  const lines = [];
  lines.push("");
  lines.push(viz.c("# codebase-intel retrieval eval", viz.colors.bold, viz.colors.cyan));
  lines.push("");
  ...
  const graphStatus = (indexedFiles > 0)
    ...
    : viz.status("warn", "empty — run: codebase-intel scan");
