#!/usr/bin/env bash
#
# Manage systemd watcher service for codebase-intel
#
# Usage:
...
SERVICE_FILE="codebase-intel-watcher@.service"
...
cmd_enable() {
  ...
  local service_name="codebase-intel-watcher@${escaped}.service"
  ...
  if [ ! -d "$project_path/.planning/intel" ]; then
    echo "Initializing codebase-intel..."
    (cd "$project_path" && codebase-intel init && codebase-intel scan)
  fi
...
cmd_list() {
  echo -e "${CYAN}Active codebase-intel watchers:${NC}"
  echo ""
  systemctl --user list-units 'codebase-intel-watcher@*' --no-pager || echo "  (none)"
}
