...
async function retrieve(root, q, opts = {}) {
  ...
  // Determine rg mode: use regex if query was expanded, otherwise respect opts
  const rgMode = (enableQueryExpansion && queryInfo.expanded !== q)
    ? "regex"
    : (opts.rgMode || "literal");
  ...
  if (backend === "zoekt") {
    try {
      ...
      searchRes = await rg.search(root, searchQuery, {
        maxHits,
        contextLines,
        mode: rgMode,
      });
