...
function isEntryPoint(relPath) {
  ...
  if (/(src\/)?(main|index|app|root|router|routes)\.(ts|tsx|js|jsx|mjs|cjs)$/i.test(relPath))
    return true;
  // WHY: Python entry points (__init__.py, __main__.py, app.py, manage.py, wsgi.py)
  // were missing — Flask's app.py got no entry point boost in cross-project testing.
...
function rerankFiles(files, { useGraphBoost = true } = {}) {
  return files.sort((a, b) => {
    ...
    // Primary: bestAdjustedHitScore (captures all scoring signals)
