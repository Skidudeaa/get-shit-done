...
codebase-intel scan --force  # index files, build graph
```
...
If you don't see the status line, codebase-intel has no way to talk to you. Check that your `statusLine` config in `~/.claude/settings.json` points to a script that reads the intel state files.
...
- **The banner** — there's a banner written to stderr on session start. You can't see it. It exists for debugging only (`echo '{"source":"startup"}' | codebase-intel hook sessionstart 2>&1 1>/dev/null`)

### How to verify it's working

1. **Status line shows `◆`** — codebase-intel is active for this project
...
5. **Run `codebase-intel summary`** — see exactly what Claude receives
6. **Run `codebase-intel health`** — see raw metrics as JSON

If the status line shows nothing after `(branch)`, either `.planning/intel/summary.md` doesn't exist (run `init` + `scan`) or the statusline script doesn't have the codebase-intel section.
...
```bash
codebase-intel watch         # foreground with dashboard
codebase-intel watch-start   # background (detached)
codebase-intel watch-stop    # kill background watcher
```
